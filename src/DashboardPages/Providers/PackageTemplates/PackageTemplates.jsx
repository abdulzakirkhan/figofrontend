...
import { useListPrepaidPackageTemplateQuery } from "../../../redux/ProvidersApi/providersApi";
...
export default function PackageTemplates() {
  ...
  const canCreatePackage=hasPermission(meData?.permissions,"package_templates","create")
  const canUpdatePackage=hasPermission(meData?.permissions,"package_templates","update")
  console.log("meData :",canCreatePackage)

  const handleView = (pkg) => {
    navigate(
      `/admin/providers/package-templates/${pkg.prepaidpackagetemplateid}`,
      {
...

    {
      ...
      renderCell: (_, row) => (
        <>
          ...
          <IconButton
            size="small"
            onClick={() => navigate("view", { state: row.raw })}
          >
            <VisibilityIcon color="primary" />
          </IconButton>
          ...
          {canUpdatePackage && (
            <IconButton
              size="small"
              onClick={() => navigate("edit", { state: row.raw })}
            >
